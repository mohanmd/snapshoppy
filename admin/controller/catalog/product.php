...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		$data['user_token'] = $this->session->data['user_token'];
$data['videolink'] = $this->db->query("SELECT  * FROM " . DB_PREFIX . "product_image p WHERE p.product_id = '" . (int)$this->request->get['product_id'] . "' and videolink_status=1")->row;
		...
		if (isset($this->request->post['buynow'])) {
			$data['buynow'] = $this->request->post['buynow'];
		} elseif (!empty($product_info)) {
			$data['buynow'] = $product_info['buynow'];
		} else {
			$data['buynow'] = 0;    // 2 in no
		}
	...
	if (isset($this->request->post['bestprice'])) {
			...
			$data['bestprice'] = 0;    // 2 in no
		}
		
		if (isset($this->request->post['holesaleprice'])) {
			$data['holesaleprice'] = $this->request->post['holesaleprice'];
		} elseif (!empty($product_info)) {
			$data['holesaleprice'] = $product_info['holesaleprice'];
		} else {
			$data['holesaleprice'] = 0;    // 2 in no
		}
