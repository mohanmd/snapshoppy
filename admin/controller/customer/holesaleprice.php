<?php
class ControllerCustomerHolesaleprice extends Controller {
	private $error = array();
...
*/
	...
	//	$data['delete'] = $this->url->link('customer/customer/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$customer_total = $this->model_customer_customer->getTotalgetholesaleprice($filter_data);



		$results = $this->model_customer_customer->getholesaleprice($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['customers'][] = array(
				...
				'units'          => $this->db->query('select CONCAT(title, " - ", unit) as units from oc_weight_class_description where weight_class_id="'.$result['units'].'"')->row['units'],
			...
			//	'ip'             => $result['ip'],
				...
				'unlock'         => $unlock,
			//	'store'          => $store_data,
				'edit'           => $this->url->link('customer/holesaleprice/edit', 'user_token=' . $this->session->data['user_token'] . '&id=' . $result['id'] . $url, true)
			);
		}
...
*/
		...
		$this->response->setOutput($this->load->view('customer/holesaleprice', $data));
	}

	
	public function edit(){
		$id=$_GET['id'];
		$this->db->query("UPDATE getholesaleprice set readstatus=1 where id=$id");
		$this->index();
	}
