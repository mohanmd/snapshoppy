(function($) {
    ...
    let $deactivateForm  = jQuery('#form-builder-deactivateForm');
    ...
    let deactivateFormSubmitted = localStorage.getItem('form-builder-deactivateFormSubmitted');
    ...
    function submitForm(deactivate_options, id) {
        ...
        data.push({name: 'action', value: 'form-builder-deactivate'});
        ...
        if (!deactivateFormSubmitted) {
            localStorage.setItem('form-builder-deactivateFormSubmitted', true);
            deactivateFormSubmitted = true;

            jQuery.post('./controller/extension/module/elfsight_form_builder.php', jQuery.param(data)).then(function(result) {
