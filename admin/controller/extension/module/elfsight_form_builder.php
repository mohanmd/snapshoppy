...
class Deactivate {
    ...
    public function __construct($data) {
        ...
        $this->app_slug = 'form-builder';
        $this->app_name = 'Form Builder';
    ...
    public function setDeactivateReasons() {
        return require ( (__DIR__) . '/form-builder-assets/reasons.php' );
    }
...
class ControllerExtensionModuleElfsightFormBuilder extends Controller {
    private $error = array();

    public $params = NULL;

    const ELFSIGHT_EMBED_URL = 'https://apps.elfsight.com/embed/form-builder/?utm_source=portals&utm_medium=opencart&utm_campaign=form-builder&utm_content=sign-up';

    public function index() {
        $this->load->language('extension/module/elfsight_form_builder');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting('module_elfsight_form_builder', $this->request->post);
        ...
        $data['action'] = $this->url->link('extension/module/elfsight_form_builder', 'user_token=' . $this->session->data['user_token'], true);

        $data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);

        if (isset($this->request->post['module_form_builder_status'])) {
            $data['module_form_builder_status'] = $this->request->post['module_form_builder_status'];
        } else {
            $data['module_form_builder_status'] = $this->config->get('module_form_builder_status');
        }

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');

        $this->response->setOutput($this->load->view('extension/module/elfsight_form_builder', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/elfsight_form_builder')) {
    ...
    public function setDeactivateReasons() {
        return require ( './controller/extension/module/form-builder-assets/reasons.php' );
    }
    ...
    public function uninstall() {
        ...
        include './controller/extension/module/form-builder-assets/popup.php';
    }
