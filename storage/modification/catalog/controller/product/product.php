...
$data['buynow'] = $product_info['buynow'];
$data['bestprice'] = $product_info['bestprice'];
$data['holesaleprice'] = $product_info['holesaleprice'];

  // custom by hari
 
 	      
		    $units=$this->model_catalog_product->getUnits();
			array_unshift($units,array('weight_class_id'=>'','title'=>'--Choose Units--','unit'=>''));
			...
			foreach ($results as $result) {
			    if($result['image']){
			         if($result['videolink_status']){
			        	$data['images'][] = array(
			       'popup' => '',
    			       'thumb' =>$result['image'],
			       'videolink' =>$result['videolink_status'],
			        	    );
...
'description'          => html_entity_decode($option['description'], ENT_QUOTES, 'UTF-8'),
	...
	public function bestprice(){
		...
		$datas=array(
		'name'        => $data['b_name'],
		'email'      => $data['b_emailid'],
		'contact'    => $data['b_contactno'],
		'product_id' => $data['product_id']
		);
		$this->db->query("INSERT into getbestprice set name='".$datas['name']."',emailid='".$datas['email']."',contactno='".$datas['contact']."',product_id='".$datas['product_id']."'");
	...
	public function holebestprice(){
		...
		'contact'    => $data['b_contactno'],
		'product_id' => $data['product_id'],
		'quantity' => $data['b_quantity'],
		'units' => $data['units']   // weight on units
		);
		$this->db->query("INSERT into getholesaleprice set name='".$datas['name']."',emailid='".$datas['email']."',contactno='".$datas['contact']."',product_id='".$datas['product_id']."',quantity='".$data['b_quantity']."',units='".$datas['units']."'");
